...
function selectTemplate(type, template) {
  if (type === 'default') {
    store.groupForm.clash_config = {
      ...
      groups: [
        { name: 'Proxy', type: 'select', proxies: [], useAllProxies: true },
        { name: 'Auto', type: 'url-test', proxies: [], useAllProxies: true, url: 'http://www.gstatic.com/generate_204', interval: 300, tolerance: 50 }
      ],
      rules: defaultRules,
      resources: [],
      raw_yaml: ''
